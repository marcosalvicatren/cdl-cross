...
def parse_f24(pdf_bytes, page_num):
    ...
    for line in text.split("\n"):
        ...
        # SEZIONE INPS: 1500 DM10/MET1 — importo può avere punto (es. 1.99900)
        m = re.match(r'^1500\s+(DM10|MET1)\s+\d+\s+\d{6}\s+([\d.]+)$', line)
        if m:
            imp = _centesimi(m.group(2).replace('.', ''))
    ...
    # DEBUG — mostra righe_raw nell'app
    st.session_state["f24_debug"] = righe_raw
...
def pagina_f24():
    ...
    if st.session_state.f24_voci:
        ...
        # DEBUG TEMPORANEO
        debug = st.session_state.get("f24_debug", [])
        if debug:
            with st.expander("🔍 DEBUG — righe estratte dal PDF"):
                for r in debug:
                    st.write(f"codice={r['codice']!r}  importo={r['importo']}  credito={r.get('credito',False)}")
